''' Modified from Reference: https://github.com/Yujun-Shi/BLIP '''
import sys, os, time
import numpy as np
...
# from rl_module.ppo_gpm_model import Policy
import pdb
...
def main():
    ...
    elif args.approach == 'gpm' in args.approach:
        log_name = '{}_{}_{}_{}_thrs_{}'.format(args.date, args.experiment, args.approach, args.seed, args.threshold)
    elif args.approach == 'sgp' in args.approach:
        log_name = '{}_{}_{}_{}_thrs_{}_coff_{}'.format(args.date, args.experiment, args.approach, args.seed, args.threshold, args.scale_coff)
    ...
    elif args.approach == 'gpm' or args.approach == 'sgp':
        print('Network bias:',args.network_bias)
        if args.network_bias:
            from rl_module.ppo_gpm_model_with_bias import Policy
        else:
            from rl_module.ppo_gpm_model import Policy
    ...
    if args.approach == 'fine-tuning' or args.approach == 'ft-fix':
        ...
        from rl_module.ppo_gpm import PPO_GPM as approach
        ...
        agent = approach(
            ...
            custom_adam=args.custom_adam,
            gpm_mini_batch=args.gpm_mini_batch,
            network_bias=args.network_bias)

    elif args.approach == 'sgp':
        from rl_module.ppo_sgp import PPO_SGP as approach
    ...
    for k, (m, param) in enumerate(agent.actor_critic.named_parameters()):
        print (m,param.shape)

    for task_idx,env_name in task_sequences:
        print(env_name)
        # renew optimizer
        agent.renew_optimizer(device)
        # Compute Projection Matrix 
        if task_idx > 0:
            agent.compute_feature_mat(device)

        envs = make_vec_envs(env_name, args.seed, args.num_processes,
                             args.gamma, args.log_dir, device, False) ## originally 'False' here !! 
        obs = envs.reset()
        ...
        print ('Iteration : ', agent.iteration)
        ...
        if args.approach == 'fine-tuning':
            ...
            envs.close()
            print (">> Finished Fisher Update after task {}".format(task_idx+1))
        elif args.approach == 'gpm':
            agent.update_GPM(rollouts, task_idx)
            envs.close()
            print (">> Finished GPM Update after task {}".format(task_idx+1))
        elif args.approach == 'sgp':
            agent.update_SGP(rollouts, task_idx, args.log_dir)
            envs.close()
            print (">> Finished GPM and Scale Update after task {}".format(task_idx+1))
        elif args.approach == 'blip':
