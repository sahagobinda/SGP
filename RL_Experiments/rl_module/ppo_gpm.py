...
class PPO_GPM():
                ...
                if self.custom_adam == True:
                    ...
                    if task_num>0:
                        ...
                        for k, (m,params) in enumerate(self.actor_critic.named_parameters()):
                            if k<4 and len(params.size())!=1: # 1st 4 layers 
    ...
    def get_representation_matrix (self, rollouts, task_num, collect_act=True):
        ...
        if self.actor_critic.is_recurrent:
            data_generator = rollouts.recurrent_generator(
                advantages, self.gpm_mini_batch)
        else:
            data_generator = rollouts.feed_forward_generator(
                advantages, self.gpm_mini_batch)
    ...
    def get_GPM (self, mat_list, task_id):
        ...
        if not feature_list:
            ...
            for i in range(len(mat_list)):
                ...
                sval_total = (S1**2).sum()
                # Projected Representation (Eq-8) - ref. GPM
                act_hat = activation - np.dot(np.dot(feature_list[i],feature_list[i].transpose()),activation)
                U,S,Vh = np.linalg.svd(act_hat, full_matrices=False)
                # criteria (Eq-9) - ref. GPM
                sval_hat = (S**2).sum()
                sval_ratio = (S**2)/sval_total               
    ...
    def update_GPM(self, rollouts, task_num):
        mat_list = self.get_representation_matrix(rollouts, task_num)
        self.feature_list = self.get_GPM(mat_list, task_num)
