...
def train_ppo(actor_critic, agent, rollouts, task_idx, env_name, task_sequences, envs, obs_shape, args,
    ...
    for j in tqdm(range(num_updates)):
        ...
        if args.use_linear_lr_decay: ### need to modify this for custom Adam - needed for GPM
            # decrease learning rate linearly
            if args.custom_adam ==True:
                utils.update_linear_schedule_custom_adam(
                    agent.optimizer, j, num_updates, args.lr)
                # print ('new_lr: ',agent.optimizer.lr)
                agent.iteration +=1
            else:
