...
class CNNBase(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=1024):
        ...
        self.conv1 = nn.Conv2d(num_inputs, 32, 8, stride=4, bias=True)
        ...
        self.conv2 = nn.Conv2d(32, 64, 4, stride=2, bias=True)
        ...
        self.conv3 = nn.Conv2d(64, 128, 3, stride=1, bias=True)
        s=compute_conv_output_size(s,3,1) # (map_size_input,k_size)
        self.map.append(s*s*128) #4

        self.fc1   = nn.Linear(128 * 7 * 7, hidden_size, bias=True)
        self.relu  = nn.ReLU()
